...
- 3D HomePage: `shared/Scene3D` (fixed overlay, z-index −1, dpr [1,2]) + `pages/HomePage/ui/scene/` (DroneModel, AkmModel, HomeScene, useHomeChoreography, types). Оркестрація — GSAP ScrollTrigger у HomePage; спільний мутабельний `Choreo` (без ре-рендерів) + `featuresStep` (React-стан для картки).
